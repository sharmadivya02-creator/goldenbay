...
function openDetail(id) {
  ...
  const rows = [
    ...
    { type: 'summary', icon: 'file', tone: 'violet', label: 'Medical summary', value: '✦ Generated by Gemini — tap to view' },
    { type: 'labs', icon: 'file', tone: 'green', label: 'Lab reports', value: 'Read, verified, and put side by side' },
  ];
  ...
  $('detailList').querySelectorAll('.drow[data-row]').forEach((btn) => {
    btn.onclick = () => {
      // Lab reports get their own full screen — charts need the room.
      if (btn.dataset.row === 'labs') {
        window.location.href = '/labs?p=' + encodeURIComponent(p.id);
        return;
      }
      openRowDetail(btn.dataset.row, p);
    };
  });
...
const ROW_META = {
  ...
  summary:    { title: 'Medical summary', icon: 'file',      tone: 'violet', docCategory: null },
};

// In-memory cache so re-opening a profile's summary within the same session
// doesn't re-call the AI every time — cleared on page reload.
const summaryCache = {};
...
function openRowDetail(type, p) {
  ...
  const TONE_COLORS = {
    red: ['var(--red-soft)', 'var(--red-dark)'],
    blue: ['var(--blue-soft)', 'var(--blue)'],
    green: ['var(--green-soft)', 'var(--green)'],
    amber: ['var(--amber-soft)', '#96701c'],
    violet: ['var(--violet-soft)', 'var(--violet)'],
  };
  const [heroBg, heroFg] = TONE_COLORS[meta.tone] || TONE_COLORS.amber;
  let body = `<div class="row-hero"><div class="ic-lg" style="background:${heroBg};color:${heroFg}">${icon(meta.icon, { size: 26 })}</div>
  ...
  if (type === 'bloodgroup') {
    ...
    body += `<div id="summaryBody">${summaryCache[p.id] ? summaryHTML(summaryCache[p.id]) : summaryLoadingHTML()}</div>`;
  }
  ...
  if (type === 'summary') loadProfileSummary(p, { force: false });

  goTab('rowdetail');
}

function summaryLoadingHTML() {
  return `<p class="muted" style="text-align:center;padding:18px 0">✦ Gemini is putting the summary together…</p>`;
}

function summaryHTML(s) {
  return `
    <p class="badge source" style="margin-bottom:12px">${s._source === 'gemini' ? '✦ Gemini' : 'mock AI'}</p>
    <p style="font-size:14.5px;line-height:1.55">${esc(s.summary)}</p>
    ${s.keyPoints?.length ? `<h3 style="margin-top:16px">Key points</h3><ul class="row-list">${s.keyPoints.map((k) => `<li class="row-item"><span class="dot"></span>${esc(k)}</li>`).join('')}</ul>` : ''}
    ${s.insuranceNote ? `<h3 style="margin-top:16px">Insurance</h3><p class="muted" style="font-size:13.5px">${esc(s.insuranceNote)}</p>` : ''}
    <p class="muted" style="margin-top:16px;font-size:11.5px">AI organises information only — it does not diagnose. Always confirm with a medical professional.</p>
    <button class="subtle" id="btnRegenSummary" style="width:100%;margin-top:10px">↻ Regenerate</button>
  `;
}

async function loadProfileSummary(p, { force }) {
  if (!force && summaryCache[p.id]) return; // already showing the cached version
  const container = $('summaryBody');
  if (container && force) container.innerHTML = summaryLoadingHTML();
  try {
    const { summary } = await api('GET', `/v1/profiles/${p.id}/summary`);
    summaryCache[p.id] = summary;
    const target = $('summaryBody');
    if (target) {
      target.innerHTML = summaryHTML(summary);
      $('btnRegenSummary').onclick = () => loadProfileSummary(p, { force: true });
    }
  } catch (err) {
    const target = $('summaryBody');
    if (target) target.innerHTML = `<p class="muted" style="text-align:center;padding:18px 0">Couldn’t generate a summary right now — try again in a moment.</p><button class="subtle" id="btnRegenSummary" style="width:100%;margin-top:10px">↻ Try again</button>`;
    if ($('btnRegenSummary')) $('btnRegenSummary').onclick = () => loadProfileSummary(p, { force: true });
  }
}
